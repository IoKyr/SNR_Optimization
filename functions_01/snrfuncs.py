...
#----------------------------------------------------------------------------------------#
def plot_snr_vs_trials_sources(p, dist_to_show=10, method_to_show='dSPM',
                                t_signal=(0.025, 0.065), t_baseline=(-0.1, 0.0),
                                n_sources_to_show=5,
                                plot_mode='snr_rank',
                                hand_disk_radius_mm=8.0):
    """
    For a single subject, fixed method and distance, plot SNR vs n_trials.

    plot_mode='snr_rank' : original behaviour — sources spanning min->max SNR
    plot_mode='distance' : sources at increasing geodesic distance from LH hand area
    """
    cwd = getcwd()
    subj = p.subj[0] if isinstance(p.subj, list) else p.subj
    print(f"Subject: {subj} | Method: {method_to_show} | Distance: {dist_to_show} mm")

    # ---- SNR helper ------------------------------------------------------
    def _snr(data, times):
        sig_mask  = (times >= t_signal[0])  & (times <= t_signal[1])
        base_mask = (times >= t_baseline[0]) & (times < 0)
        rms = np.sqrt(np.mean(data[:, sig_mask] ** 2, axis=1))
        std = np.std(data[:, base_mask], axis=1)
        std[std == 0] = 1e-10
        return rms / std
    ...
    stc_list = []
    for num in p.reps:
        fname = f"current{num}reps_{dist_to_show}mm_{method_to_show}-lh.stc"
        fpath = path.join(cwd, p.data, p.processed_data, p.protocol,
                          subj, p.session, p.current_dir, fname)
        stc_list.append(mne.read_source_estimate(fpath))

    times      = stc_list[0].times
    lh_vertno  = stc_list[0].vertices[0]
    ...
    if plot_mode == 'distance':
        ...
        # 1. Find hand centre vertex on lh.white via HCPMMP1 S1 labels ----
        surf_path = path.join(p.subj_brains_dir, p.fs_subject_dir, 'surf', 'lh.white')
        coords, faces = mne.read_surface(surf_path)

        labels_all = mne.read_labels_from_annot(
            subject=p.fs_subject_dir, parc='HCPMMP1',
            hemi='lh', subjects_dir=p.subj_brains_dir
        )
        s1_names = ['L_3b_ROI-lh', 'L_1_ROI-lh', 'L_2_ROI-lh']
        s1_verts = np.unique(np.hstack(
            [lbl.vertices for lbl in labels_all if lbl.name in s1_names]
        ))
        centroid         = coords[s1_verts].mean(axis=0)
        hand_center_vert = int(np.argmin(np.linalg.norm(coords - centroid, axis=1)))
        print(f"Hand centre vertex: {hand_center_vert}  "
              f"pos={coords[hand_center_vert].round(1)}")

        # 2. Build cortical graph and compute geodesic distances -----------
        edges = np.vstack([faces[:, [0,1]], faces[:, [1,2]], faces[:, [2,0]]])
        edges = np.vstack([edges, edges[:, ::-1]])
        edge_len = np.linalg.norm(coords[edges[:,0]] - coords[edges[:,1]], axis=1)
        graph = coo_matrix((edge_len, (edges[:,0], edges[:,1])),
                           shape=(len(coords), len(coords)))

        geo_dists_full = dijkstra(csgraph=graph, directed=False,
                                  indices=hand_center_vert)

        # 3. Keep only distances for vertices actually in this STC ---------
        geo_dists_stc = geo_dists_full[lh_vertno]

        finite_mask  = np.isfinite(geo_dists_stc)
        max_dist     = min(geo_dists_stc[finite_mask].max(), 150.0)
        target_dists = np.linspace(0, max_dist, n_sources_to_show)

        source_ids = []
        dist_vals  = []
        seen       = set()
        for td in target_dists:
            masked = np.where(finite_mask, np.abs(geo_dists_stc - td), np.inf)
            idx    = int(np.argmin(masked))
            if idx not in seen:
                source_ids.append(idx)
                dist_vals.append(geo_dists_stc[idx])
                seen.add(idx)

        labels = [f"src {sid} [d={dv:.0f}mm]"
                  for sid, dv in zip(source_ids, dist_vals)]

        print("Selected sources (by distance):")
        for lbl in labels:
            print(f"  {lbl}")

        # 4. Compute SNR curves using only LH data -------------------------
        snr_curves = {src: [] for src in source_ids}
        for stc in stc_list:
            lh_data = stc.data[:len(lh_vertno)]
            snr_all = _snr(lh_data, times)
            for src in source_ids:
                snr_curves[src].append(snr_all[src])

        xlabel_str   = "Number of trials"
        title_str    = (f"SNR vs. n_trials  |  {subj}  |  {method_to_show}  "
                        f"|  {dist_to_show} mm  |  LH distance from hand")
        legend_title = "Source (geodesic dist)"

    # ======================================================================
    # SNR-RANK-BASED source selection (original behaviour)
    # ======================================================================
    else:
        ref_snr     = _snr(stc_list[-1].data, times)
        snr_min_idx = int(np.argmin(ref_snr))
        snr_max_idx = int(np.argmax(ref_snr))
        target_snrs = np.linspace(ref_snr[snr_min_idx], ref_snr[snr_max_idx],
                                  n_sources_to_show)
        source_ids  = [int(np.argmin(np.abs(ref_snr - t))) for t in target_snrs]

        labels = []
        for i, src in enumerate(source_ids):
            snr_val = ref_snr[src]
            tag = ("min" if i == 0 else
                   "max" if i == len(source_ids) - 1 else
                   f"p{int(100 * i / (len(source_ids) - 1))}%")
            labels.append(f"src {src} [{tag}, SNR={snr_val:.1f}]")
    ...
    palette = sns.color_palette("rocket_r", len(source_ids))
    markers = ['o', 's', '^', 'D', 'v', 'P', 'X', '*', 'h', 'p']

    fig, ax = plt.subplots(figsize=(8, 5))

    for src, label, color, marker in zip(source_ids, labels, palette, markers):
        ax.plot(p.reps, snr_curves[src],
                color=color, marker=marker,
                linewidth=2.2, markersize=6, label=label)

    ax.set_xlabel(xlabel_str, weight='bold')
    ax.set_ylabel("SNR (signal RMS / baseline std)", weight='bold')
    ax.set_title(title_str, weight='bold')
