...
def plot_same_source_different_methods(p, remove_outliers=False):
    ...
    # ------------------ SNR CALCULATION ------------------
    def calculate_snr(stc, times, method, dist):
        ...
        signal_rms = np.sqrt(np.mean(data[:, signal_mask] ** 2, axis=1))
        
        # Calculate std for baseline
        baseline_std = np.std(data[:, baseline_mask], axis=1)
    ...
    # Find highest SNR source for each subject
    print("\nCalculating SNR for each subject...")
    highest_snr_sources_fig1 = {}  # uses reference_dist=10, for Figure 1 only
    highest_snr_sources = {}       # uses reference_dist=20, for Figures 2 & 3 to ensure source ID exists in all spaces
    
    for subj in subjects:
        for ref_dist, store in [(10, highest_snr_sources_fig1), (20, highest_snr_sources)]:
            reference_rep_idx = reps.index(2000)
            all_snrs = []
            for method in methods:
                stc = stcs[subj][ref_dist][method][reference_rep_idx]
                snr = calculate_snr(stc, stc.times, method, ref_dist)
                all_snrs.append(snr)
            avg_snr = np.mean(all_snrs, axis=0)
            max_snr_idx = np.argmax(avg_snr)
            store[subj] = max_snr_idx
            print(f"Subject {subj} (ref {ref_dist}mm): highest SNR source = {max_snr_idx}, SNR = {avg_snr[max_snr_idx]:.2f}")

    # For Figure 1
    subi = 0  # Use i+1 subject
    fig1_subject = subjects[subi]
    max_source_idx = highest_snr_sources_fig1[fig1_subject]
    ...
    # ------------------ Figure 1: STC Activity (using highest SNR source) ------------------
    ...
    with sns.plotting_context("notebook", font_scale=font_scale):
        ...
        # First pass: collect y-limits from dSPM and sLORETA 
        ref_ymins, ref_ymaxs = [], []
        for method in ['dSPM', 'sLORETA']:
            method_data = -df[df["Method"] == method]["Activity"]
            ref_ymins.append(method_data.min())
            ref_ymaxs.append(method_data.max())
        
        ref_ymin = min(ref_ymins)
        ref_ymax = max(ref_ymaxs)
        ref_range = ref_ymax - ref_ymin
        ref_center = (ref_ymax + ref_ymin) / 2

        for idx, (ax, method) in enumerate(zip(axes, methods)):
            for i, rep in enumerate(reps):
                data = df[(df["Method"] == method) & (df["Repetitions"] == rep)]
                ax.plot(
                    data["Time (s)"], -data["Activity"],
                    color=gray_colors[i],
            ...
            if method in ['dSPM', 'sLORETA']:
                method_data = -df[df["Method"] == method]["Activity"]
                m_min, m_max = method_data.min(), method_data.max()
                abs_max = max(abs(m_min), abs(m_max))
                padding = abs_max * 0.15
                ax.set_ylim(-abs_max - padding * 0.3, abs_max + padding)
                ...
                padding = abs_max * 0.15
                ax.set_ylim(-abs_max - padding * 0.3, abs_max + padding)
                ax.set_ylabel("Current (A·m)", weight='bold')
        ...
        for dist in dists:
            for method in methods:
                for i, stc in enumerate(stcs[subj][dist][method]):
                    ...
                    baseline_var = np.var(source_ts[baseline_mask]) if np.any(baseline_mask) else np.nan
                    baseline_var_data.append({
                        'Subject': subj,
                        'Distance (mm)': dist,
                        'Method': method,
                        'Repetitions': reps[i],
                        'Baseline Variance': baseline_var
                    })

    df_var = pd.DataFrame(baseline_var_data)
    df_var['Method'] = pd.Categorical(df_var['Method'], categories=methods, ordered=True)
    ...
    df_grouped = (
        ...
        .agg(
            mean_var=('Baseline Variance', 'mean'),
            sem_var=('Baseline Variance', lambda x: np.std(x, ddof=1) / np.sqrt(len(x)) if len(x) > 0 else 0)
        )
    )
            ...
            for method in methods:
                ...
                for i in range(len(reps) - 1):
                    ...
                    for subj in subjects:
                        ...
                        val1 = subj_data.loc[subj_data['Repetitions'] == rep1, 'Baseline Variance']
                        val2 = subj_data.loc[subj_data['Repetitions'] == rep2, 'Baseline Variance']
    ...
    with sns.plotting_context("notebook", font_scale=font_scale):
        ...
        fig2, axes2 = plt.subplots(2, 2, figsize=(14, 10), sharey=False, sharex=False)
        axes2 = axes2.flatten()
        ...
        for j, method in enumerate(methods):
            ...
            for idx, dist in enumerate(dists):
                dist_data = df_grouped[
                    (df_grouped['Method'] == method) &
                    (df_grouped['Distance (mm)'] == dist)
                ].copy().sort_values('Repetitions')

                dist_data['rep_idx'] = range(len(dist_data))

                ax.errorbar(
                    dist_data['rep_idx'], dist_data['mean_var'],
                    yerr=dist_data['sem_var'],
            ...
            ax.set_xticklabels(reps, rotation=45)
            ax.set_title(method, weight='bold')
            ax.set_xlabel("Number of trials", weight='bold')
            ax.set_ylabel("Baseline Variance", weight='bold')
            ...
            for spine in ax.spines.values():
                spine.set_linewidth(1.2)
                spine.set_color('black')
            ax.tick_params(axis='both', length=5, width=1.2, color='black', labelcolor='black')
            ax.grid(False)

            # --- Significance annotations ---
            if len(df_sig) > 0 and 'pval_fdr' in df_sig.columns:
                method_sig = df_sig[df_sig['Method'] == method]

                for dist in dists:
                    dist_sig = method_sig[method_sig['Distance (mm)'] == dist]
                    dist_data = df_grouped[
                        (df_grouped['Method'] == method) &
                        (df_grouped['Distance (mm)'] == dist)
                    ].copy().sort_values('Repetitions')
                    ...
                    dist_idx = dists.index(dist)

                    for _, row in dist_sig.iterrows():
                        pval = row.get('pval_fdr', row['pval'])
                        if pd.isna(pval):
                            continue
                        is_sig = row.get('significant_fdr', pval < 0.05)
                        if not is_sig:
                            continue
                        ...
                        # x position = index of Rep2 in reps
                        rep2_idx = reps.index(row['Rep2'])
                        y_val = dist_data[dist_data['Repetitions'] == row['Rep2']]['mean_var'].values
                        if len(y_val) == 0:
                            continue

                        ax.text(
                            rep2_idx, y_val[0] * 1.15,
                            mark,
                            ha='center', va='bottom',
                            color=gray_colors[dist_idx],
        ...
        plt.suptitle("Baseline Variance per Method", weight='bold', y=1.02)
        plt.tight_layout()
        plt.subplots_adjust(right=0.88, wspace=0.35, hspace=0.4)
        plt.show()
    ...
    if remove_outliers:
        mask = iqr_filter(df_var, "Baseline Variance")
    ...
    df_grouped = (
        df_var.groupby(['Distance (mm)', 'Method', 'Repetitions'], as_index=False, observed=True)
        .agg(mean_var=('Baseline Variance', 'mean'),
             sem_var=('Baseline Variance', lambda x: np.std(x, ddof=1)/np.sqrt(len(x)) if len(x) > 0 else 0))
    )
    
    df_grouped['Method'] = pd.Categorical(df_grouped['Method'], categories=methods, ordered=True)
    ...
    with sns.plotting_context("notebook", font_scale=font_scale):
        ...
        for j, method in enumerate(methods):
            ...
            method_data = df_grouped[
                (df_grouped['Method'] == method) &
                (df_grouped['Distance (mm)'] == target_dist)
            ].copy().sort_values('Repetitions')

            method_data['rep_idx'] = range(len(method_data))

            ax.errorbar(
                method_data['rep_idx'], method_data['mean_var'],
                yerr=method_data['sem_var'],
                fmt='-D', markersize=10, linewidth=2.5, capsize=5,
                color='black',
                label=f'{target_dist} mm'
            )
            ...
            # y-axis label depends on row
            if row == 0:
                ax.set_ylabel("Baseline Variance (z²)", weight='bold')
            else:
                ax.set_ylabel("Baseline Variance (A²m²)", weight='bold')
            ...
            if len(df_sig) > 0 and 'pval_fdr' in df_sig.columns:
                method_sig = df_sig[
                    (df_sig['Method'] == method) &
                    (df_sig['Distance (mm)'] == target_dist)
                ]
                ...
                for _, row in method_sig.iterrows():
                    ...
                    rep2_idx = reps.index(row['Rep2'])
                    y_val = method_data[method_data['Repetitions'] == row['Rep2']]['mean_var'].values
                    if len(y_val) == 0:
                        continue
        ...
        plt.suptitle(f"Baseline Variance per Method ({target_dist} mm)", weight='bold', y=1.02)
        plt.tight_layout()
        ...
        for dist in dists:
            for method in methods:
                for i, stc in enumerate(stcs[subj][dist][method]):
                    ...
                    total_power = np.sum(source_ts[time_mask] ** 2)
                    power_data.append({
                        'Subject': subj,
                        'Distance (mm)': dist,
                        'Method': method,
    ...
    with sns.plotting_context("notebook", font_scale=font_scale):
        for i_dist, dist in enumerate(dists):
            ...
            for j_method, method in enumerate(methods):
                ax = axes[j_method]
                subplot_row = j_method // 2
                method_data = row_data[row_data['Method'] == method].sort_values('Repetitions')

                # Compute per-method scale exponent
                method_max = method_data['mean_power'].max()
                scale_exp = 3 * (int(np.floor(np.log10(method_max))) // 3)
                scale_factor = 10 ** scale_exp
                method_data = method_data.copy()
                method_data['mean_power_scaled'] = method_data['mean_power'] / scale_factor
                method_data['sem_power_scaled'] = method_data['sem_power'] / scale_factor
                ...
                # y-axis label depends on row
                if subplot_row == 0:
                    ax.set_ylabel('Total Power (z²)', weight='bold')
                else:
                    ax.set_ylabel('Total Power (A²m²)', weight='bold')
                ...
                # Per-method y-limits with 5% padding
                m_min = (method_data['mean_power_scaled'] - method_data['sem_power_scaled']).min()
                m_max = (method_data['mean_power_scaled'] + method_data['sem_power_scaled']).max()
                m_range = m_max - m_min
                ax.set_ylim(m_min - m_range * 0.05, m_max + m_range * 0.05)

                # Scale exponent annotation
                ax.text(
